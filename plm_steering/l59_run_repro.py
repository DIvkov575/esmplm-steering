"""L59: activation-steering run for transmembrane-residue fraction, reusing
...
Run:  L59_SEED=0 python3 -m plm_steering.l59_run_repro   (seeds 0,1,2 for robustness)
"""
import json
import os
...
from plm_steering.l59_transmembrane_steering import (
    transmembrane_proxy,
    transmembrane_proxy_excluding,
)

MODEL_NAME = "facebook/esm2_t33_650M_UR50D"
DATA_PATH = Path(__file__).resolve().parent / "data_cache" / "transmembrane" / "uniprot_tm.json"
SEED = int(os.environ.get("L59_SEED", "0"))
OUT_DIR = Path(__file__).resolve().parent / ("l59_repro_out" if SEED == 0 else f"l59_repro_out_seed{SEED}")
...
def score_transmembrane(sequences):
    return np.array([transmembrane_proxy(seq) for seq in sequences])


def load_transmembrane(path=DATA_PATH, max_len=MAX_SEQ_LEN):
    """UniProt records -> (sequences, tm_fraction) deduplicated by sequence."""
    with open(path) as f:
        records = json.load(f)
    by_sequence = {}
    for rec in records:
        ...
        if len(seq) > max_len or len(seq) == 0 or not set(seq) <= CANONICAL_RESIDUES:
            continue
        by_sequence[seq] = float(rec["tm_fraction"])  # unique per accession already
...
def main():
    ...
    sequences, labels = load_transmembrane()
    ...
    train_proxy = score_transmembrane(train_seqs)
    test_proxy = score_transmembrane(test_seqs)
    ...
    print(f"proxy vs real tm_fraction: train r={r_train:+.4f} (p={p_train:.2e}), "
    ...
    print(f"vector-building groups: {len(low_group)} low-tm (<= {low_threshold:.3f}), "
          f"{len(high_group)} high-tm (>= {high_threshold:.3f})", flush=True)
    ...
    print(f"eval sequences (low-tm, held-out): {len(eval_sequences)}", flush=True)
    ...
    def generate_then_score(vectors, alpha):
        ...
        return generated, score_transmembrane(generated)
    ...
    if best_alpha is not None:
        ...
        real_excl_scores = np.array([
            transmembrane_proxy_excluding(s, top_residues) for s in np.array(real_generated)[keep]
        ])
        random_excl_scores = np.array([
            transmembrane_proxy_excluding(s, top_residues) for s in np.array(random_generated)[keep]
        ])
    ...
    crit5 = None  # no prior transmembrane-steering technique exists
    ...
    print("\n=== L59 VERDICT (transmembrane fraction, degenerate-filtered, "
