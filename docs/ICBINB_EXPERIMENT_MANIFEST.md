...
Status: Execution contract. Freeze this file by commit hash before building
the final audit bundles. A result does not pass merely because it matches a
number already present in a narrative document.
...
This manifest turns the approved minimum ICBINB evidence package into a fixed
audit. The audit asks which checks prevent an apparent protein language model
steering result from being interpreted as reliable biological property
control.
...
|---|---|---|---|
| L42 | Optional recovery | Additional decoder-instability example | Admit only if a raw audit bundle is regenerated by 2026-08-15 23:59 Anywhere on Earth. Otherwise exclude. |
...
| L51 | Optional recovery | Additional decoder-instability example | Admit only if a derived bundle records the original and corrected policies and raw sequences, scores, failure flags, configuration, seeds, and source revision by the L42 cutoff. Otherwise exclude. |
| L52 | Required | Primary decoder-instability case | Include after the audit bundle passes H1 and artifact verification. |
...
| L55 | Required | Seed-sensitive composition case | Include only after seeds 0, 1, and 2 are reproduced with explicit seed and output metadata. |
| L56 | Required | Primary endpoint-mismatch and grouped-validation sensitivity case | Include after the cached-data validation and grouping-sensitivity checks pass. No steering run is permitted. |
| L57 | Required | Primary composition-sensitivity case | Include after the two-part generation audit and residue-exclusion analysis pass H4. |
...
The minimum package does not wait for L42, L51, L53, or any Interp4Discovery
experiment.
...
unavailable for all current saved runs and is not required unless a rerun
saves logits.
...
Before H1 analysis, rebuild the ordered evaluation cohort from
`plm_steering/data_cache/meltome/mixed_split.csv`, whose current SHA-256 is
`ea48dbb222a5e18de61fabf0e1550b50e7a22c3d0f59cf9b47c01d993104ea3e`,
using the frozen runner revision and seed. Save the source identifier,
source-sequence hash, and array index for every one of the 60 rows. Verify
that arm arrays preserve this order. If the mapping cannot be reconstructed
and independently checked, rerun L52 with source identifiers. A row number
without this reconstruction is not an auditable protein identifier.
...
H3 is accepted only if all conditions hold:

- All three seed bundles record the seed explicitly and contain 150 attempted
  generations per arm and alpha.
...
- The paper reports the exact pass count. It may say `two of three` only if
  exactly two reruns pass.
...
H3 fails if all three artifact-check verdicts agree, if any seed bundle cannot
be reproduced, or if a seed is inferred only from its directory name. If H3
fails, the required third mechanism fails and ICBINB-BIO does not pass its
submission gate. L57 alone does not replace H3.
...
Run all commands from the repository root in an isolated clean worktree
created from the frozen manifest commit.
...
Stop if tracked files are dirty, the manifest is not committed, an input hash
differs without an approved provenance update, or the model revision is not
immutable.
...
Add and pass focused tests for generation validity, failure-risk
denominators, policy replay, seed metadata, and audit-schema validation
before final execution.

### 13.3 Required runner interfaces

The following target interfaces are required before rerunning model
experiments. They are not implemented. The legacy entry points abort through
`plm_steering.legacy_runner_guard`, so these commands fail closed until strict
argument parsers, safe output checks, metadata writers, and command-interface
tests exist.
...
### 13.4 Optional L42 and L51 recovery

These commands are optional and their interfaces are not implemented in the
current source. Run them only if they can finish before the fixed cutoff.
...
### 13.5 Audit build and verification

The required audit module interface is:
...
These gaps are unresolved at manifest creation and block final execution:
...
The orchestrator must record the resolution or accepted claim removal for
each gap before locking the result ledger.
