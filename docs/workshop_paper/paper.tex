...
\begin{abstract}
Activation steering has recently been shown to reproduce thermostability
in ESM2-650M using vectors formed from mean activation differences. We ask two questions that
this result does not answer. Does a scoring proxy's correlation with real
experimental labels predict whether steering toward it works? Can a
single steering run be trusted at face value? Under a pre-registered
six-criterion protocol, we test five novel target properties: binding
affinity, catalytic activity, intrinsic disorder, immunogenicity, and
expression yield. We find no positive
...
causal ablation effect. A single-seed steering result can also mislead in
criterion-specific ways. For one target, the effect direction replicates
across three seeds, but the residue-exclusion artifact check passes on
only two. We also introduce a lightweight diagnostic based on
steering-vector cosine similarity across targets. It explains an
otherwise unexplained ambiguous result as a geometric echo of a different
property's validated direction. We release the full
...
difference between high- and low-property example groups. This method has
been shown to reproduce known thermostability biology in ESM2-650M
\citep{huang2025steering}. The result raises a natural question for a new
target property: does the strength of a scoring proxy's correlation with
real experimental labels predict whether steering toward it will work?

We find that the answer is clearly no. Across five novel
...
a low-correlation control head. A full sweep of all 480 heads in the
model shows that the correlational top pick ranks 313th of 480 by real
causal effect.

At two levels and with two independent techniques, activation steering
and attention-head ablation, we find the same lesson. A correlational
signal is evidence about a model's representations, not about what
happens after an intervention. This paper's contribution is not the
steering technique itself, which we borrow unchanged from prior work, but
the audit discipline needed to trust a causal claim in this domain. We
demonstrate that discipline through two near-misses caught in our own
...
residues than steering a random latent. This is a coherence check, not a
...
\begin{enumerate}
\item Beats both controls head-to-head with a real confidence interval.
  The real steering direction must significantly beat, via
  direct paired bootstrap (10{,}000 resamples), a matched-norm
  random-direction control, rather than pass two separate comparisons against an
  unsteered baseline.
\item Shows a dose/scale-response rather than a one-off effect. The effect must show
  ...
  range. Section~\ref{sec:l52} explains why this restriction matters.
\item Survives a residue-exclusion robustness check. If the
  ...
  diminished.
\item Uses a proxy validated against real labels before the steering run.
  No proxy is used in a PASS decision without first
  being checked against real experimental labels, ideally on a held-out
  split.
\item Beats the best-known existing technique when one exists for the
  target property. This criterion does not apply to a genuinely new
  property with no prior baseline.
\item Is adequately powered, with at least 150 held-out evaluation
  sequences for any claim involving a new target property.
\end{enumerate}

A target that fails any criterion outright is a KILL. A target
that passes 1--2 criteria but fails on 3 or 4 is
AMBIGUOUS. Such a result is informative but not actionable. All five novel
...
\section{Component Evidence: Correlation Does Not Imply Causal Necessity}
\label{sec:component}
...
show exactly zero measurable causal effect. This is real redundancy in
...
\label{tab:sweep}
\small
\begin{tabular}{@{}p{2.5cm}p{2.6cm}p{1.2cm}p{3.4cm}p{2.2cm}@{}}
\toprule
...
$\alpha$, with zero degenerate sequences at any strength. This rules out
generation collapse as an alternative explanation for the null.

A plausible mechanistic reason is that the vector-building groups in this
dataset are single-backbone point mutants. They differ by 1--2 residues
from a shared 188-residue reference, which gives the difference-of-means
construction very little raw compositional signal to use. This limitation
is independent of whether binding affinity is causally steerable in this
model. We measure it directly: the mean per-residue
...
Using DLKcat, a cross-protein enzyme turnover-number dataset spanning many
organisms and EC classes, we build a proxy from the fraction of glycine
minus the fraction of arginine in a sequence. This matches the published
activity-stability tradeoff in the enzymology
...
$r=0.22$ held-out, the weakest result among the four runnable targets in
our sweep. Yet it produces a clean, monotonic dose-response effect,
significant at every tested $\alpha$ in the safe range. The effect
retains roughly one third of its magnitude after the two dominant
substituted residues are excluded. One of those residues is also one of
the proxy's two defining terms. We reran the
...
We reduce DisProt's real per-residue intrinsic-disorder annotations to a
per-sequence scalar. We then build a proxy from the published TOP-IDP
amino-acid disorder-propensity scale \citep{campen2008topidp}. It
validates at $r=0.45$ held-out, the strongest result for any target in
our sweep. A sliding-window analysis against real per-residue disorder
labels gives an AUC of $0.71$. The
...
collapse is not uniformly stable. We report this as a majority pass with
an explicit caveat, not a unanimous clean result. It also shows that
criteria within the same protocol can have different seed sensitivity,
which no single run can reveal.
...
model run was performed. This is an intended KILL under the protocol, not
missing work.
...
We use eSol, a dataset of real E.\ coli soluble-expression-yield
measurements. Its labels are statistically distinct from those of an
unrelated solubility dataset: the 441 overlapping sequences show zero
correlation between the two labels. We build a proxy from the absolute
deviation of net charge from neutrality, following a published
recombinant-solubility discriminant \citep{wilkinson1991solubility}. This
proxy validates at $r=0.31$--$0.34$ held-out and produces a significant,
dose-responsive steering effect. However, the effect collapses to statistical
...
three deepest layers. The relationship is biologically plausible because
disordered protein regions are documented to reduce solubility and
expression yield. This context reframes the result. Rather than provide
...
\geq 1.0$. It reported a clean PASS: the 5-layer subset appeared to
...
repetitive output, and zero non-degenerate evaluation sequences remained.
The 5-layer subset had not yet collapsed. The subset ``won'' only
because it broke down later, not because it steered harder in any
comparable regime.

We fixed this by restricting the $\alpha$ range used for this comparison
to a pre-established safe operating window in which neither arm has
collapsed. We applied the same restriction to criterion 2's dose-response
check and reran the full experiment end to end rather than patching the
already computed numbers. The corrected result: the 5-layer subset
...
every $\alpha$ where both arms are trustworthy. This is an informative
partial result, not the unqualified success suggested by the uncorrected
analysis.
...
on seed 1. The underlying effect is the same, but the criterion produces
...
\paragraph{The vector-geometry diagnostic generalizes.} We explained our
expression-yield result (\S\ref{sec:expression}) by computing cosine
similarity between a new target's steering vector and every
previously validated target's vector. This check is cheap because it
requires one additional embedding pass, not a full steering sweep. We
suggest it is worth
...
selected as the best of several candidates, typically 7 to 9, scored on
the same held-out split whose $r$ is then reported as the validation
number. We did not use a separate train/select/test split. This
garden-of-forking-paths bias optimistically inflates every reported
proxy-$r$ in the same direction, although it does not by itself
...
different levels of the same class of models. This holds both for an
attention head's alignment with a real biological signal and for a
scoring proxy's correlation with real experimental labels. The
