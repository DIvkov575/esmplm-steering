...
\label{tab:sweep}
\small
\begin{tabular}{@{}p{2.1cm}p{2.6cm}p{1.3cm}p{3.6cm}p{1.9cm}@{}}
\toprule
...
Intrinsic disorder & DisProt & 0.45 & PASS, 3/3 direction; 2/3 robustness & \textbf{PASS} (majority) \\
...
the intrinsic-disorder target's vector (\S\ref{sec:disorder}): $+0.30$
on the full 33-layer concatenation, rising to $+0.40$ to $+0.50$ at the
...
(catalytic activity, intrinsic disorder) but not all five; the
seed-sensitivity finding in \S\ref{sec:disorder} may or may not
generalize to the other three. We did not run any structural or
functional plausibility check (e.g.\ predicted-fold confidence) on
generated sequences, so a PASS result does not rule out that steered
sequences, while scoring well on the proxy, are otherwise implausible
proteins.
